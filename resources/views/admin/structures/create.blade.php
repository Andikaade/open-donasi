<x-admin-layout>
    <div class="p-2 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Anggota Baru</h1>
                <p class="text-sm text-slate-500 mt-1">Tambahkan pengurus, pembina, atau majelis guru ke dalam struktur organisasi.</p>
            </div>
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" maxlength="10"
                            placeholder="Contoh: BED (Otomatis dibuat jika dikosongkan)"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select name="category" id="category" required
                            ...
                            <option value="pelindung" {{ old('category') == 'pelindung' ? 'selected' : '' }}>Pelindung / Penasehat</option>
                            <option value="eksekutif" {{ old('category') == 'eksekutif' ? 'selected' : '' }}>Pimpinan Eksekutif / Pembina</option>
                            <option value="pengurus_harian" {{ old('category') == 'pengurus_harian' ? 'selected' : '' }}>Pengurus</option>
                            <option value="seksi" {{ old('category') == 'seksi' ? 'selected' : '' }}>Seksi-Seksi</option>
                            <option value="guru" {{ old('category') == 'guru' ? 'selected' : '' }}>Majelis Guru</option>
                        </select>
                        @error('category')
                            <p class="text-rose-500 text-xs font-medium">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-1.5">
                        <label for="email_or_phone" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">
                            No. HP / E-mail <span class="text-slate-400 font-normal">(Opsional)</span>
                        </label>
                        <input type="text" name="email_or_phone" id="email_or_phone" value="{{ old('email_or_phone') }}"
                            placeholder="Contoh: 081234567890 / admin@gmail.com"
                    ...
                    <div class="space-y-1.5">
                        ...
                        <input type="number" name="order_priority" id="order_priority" value="{{ old('order_priority', 1) }}" min="1"
