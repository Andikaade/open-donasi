<x-admin-layout>
    <div class="p-2 space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <nav class="flex mb-2 text-xs text-slate-500 font-medium">
                    ...
                    <span class="text-slate-800 font-semibold">Tambah Artikel</span>
                </nav>
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Buat Artikel Baru</h1>
                <p class="text-sm text-slate-500 mt-1">Lengkapi formulir di bawah ini untuk mempublikasikan artikel atau berita baru.</p>
            </div>
            <div>
        ...
        <!-- Form Tambah Artikel -->
        <form action="{{ route('admin.artikels.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
                ...
                <div class="lg:col-span-8 space-y-6">
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Masukkan judul artikel atau berita..."
                        ...
                        <div>
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3" placeholder="Tulis ringkasan singkat artikel..."
                                      class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all">{{ old('excerpt') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="body" id="body" rows="12" required placeholder="Tulis narasi lengkap artikel di sini..."
                                      class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 transition-all">{{ old('body') }}</textarea>
                            @error('body')
                                <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="lg:col-span-4 space-y-6">
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-5">
                        ...
                        <div>
                            ...
                            <div class="border-2 border-dashed border-slate-200 rounded-xl p-4 text-center bg-slate-50/50 hover:bg-slate-50 transition-all">
                                ...
                                <label for="image" class="cursor-pointer space-y-2 block">
                                    <div id="image-preview-container" class="w-full h-40 rounded-lg bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                                        <div id="placeholder-text" class="text-slate-400 text-xs flex flex-col items-center gap-2">
                            ...
                            <p class="text-[11px] text-slate-400 mt-1">Format: JPG, PNG, GIF, WEBP (Max: 2MB).</p>
                            @error('image')
                                <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-400 mt-1">Kosongkan jika ingin menyimpannya sebagai draft tanpa tanggal rilis.</p>
                            @error('published_at')
                                <p class="text-xs text-rose-500 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
